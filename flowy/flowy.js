var flowy = function(root_element,canvas, grab, release, snapping, rearrange, spacing_x, spacing_y) {
    if (!root_element) {
    ...
    flowy.load = function() {
        ...
        flowy.endDrag = function(event) {
            if (event.which != 3 && (active || rearrange)) {
                dragblock = false;
                blockReleased();
                // if (!root_element.querySelector(".indicator")) {
                //     canvas_div.innerHTML = "<div class='indicator invisible'></div>";
                // }
        ...
        // root_element.off();
        console.log('end function ========================',root_element);
        if (!loaded){
            loaded = true;

            root_element.addEventListener("mousedown", flowy.beginDrag);
            root_element.addEventListener("mousedown", touchblock, false);
            root_element.addEventListener("touchstart", flowy.beginDrag);
            root_element.addEventListener("touchstart", touchblock, false);
            
    
            root_element.addEventListener("mouseup", touchblock, false);
            root_element.addEventListener("mousemove", flowy.moveBlock, false);
            root_element.addEventListener("touchmove", flowy.moveBlock, false);
    
            root_element.addEventListener("mouseup", flowy.endDrag, false);
            root_element.addEventListener("touchend", flowy.endDrag, false);
        }
    ...
    function addEventListenerMulti(type, listener, capture, selector) {
        var nodes = root_element.querySelectorAll(selector);
